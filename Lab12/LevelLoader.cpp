		...
		for (rapidjson::SizeType i = 0; i < actors.Size(); i++)
		{
			...
			if (actorValue.IsObject())
			{
				...
				else if (type == "Checkpoint")
				{
					...
					if (GetStringFromJSON(actorValue, "level", levelString))
					{
						checkpoint->SetLevelString(levelString);
					}
					if (GetStringFromJSON(actorValue, "text", checkpointString))
					{
						checkpoint->SetCheckpointString(checkpointString);
					}
					actor = checkpoint;
				}
				else if (type == "Coin")
