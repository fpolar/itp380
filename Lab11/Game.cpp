...
void Game::LoadData()
{
	...
	Mix_PlayChannel(-1, GetSound("Assets/Sounds/Music.wav"), -1);
}
...
void Game::PushCheckpoint(Checkpoint *c){
    if(mCheckpoints.size() == 0){
        c->active = true;
        c->GetMesh()->SetTextureIndex(0);
    }
    mCheckpoints.push(c);
}

void Game::PopCheckpoint(){
    mCheckpoints.front()->SetState(Actor::State::EDead);
    mCheckpoints.pop();
}

void Game::LoadNextLevel() {
//	o Delete all the actors in the world
//		o Clear out the checkpoint queue(just in case)
//		o Call LevelLoader::Load on mNextLevel
//		o Do whatever you need to do to activate the first checkpoint
//		o Allocate a new Arrow actor(since the old one got deleted)
//		o mNextLevel.clear() (so we don’t try to change the level again on the next frame)
