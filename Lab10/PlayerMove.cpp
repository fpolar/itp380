...
//    The player’s facing is roughly perpendicular to the side (keep in mind that the
//                                                              wall might be on your left or your right). You should be very lenient as to what
//    “perpendicular” is, since the player usually will come in at an acute angle
//    o The xy-velocity has you roughly moving in the player’s forward direction (so you
//                                                                                can’t backwards wall run)
//    o The xy-velocity length is > 350.0f (so you’re moving quickly enough)
...
//    std::cout << "Run " << 3 * Math::Pi/4 << "- Angle between forward and block side: " << angle << std::endl;
//    std::cout << "X: " << (mVelocity.x * mOwner->GetForward()).x << std::endl;
//    std::cout << "Y: " << (mVelocity.y * mOwner->GetForward()).y << std::endl;
//    std::cout << "Length: " << (mVelocity*Vector3{1, 1, 0}).Length() << std::endl;
...
bool PlayerMove::CanWallClimb(PlayerMove::CollSide side){
    ...
    normal_forward.Normalize();
    float angle = Math::Acos(Vector3::Dot(normal_forward, normal));;
//    std::cout << "Climb " << 3*Math::Pi/4 << "- Angle between forward and block side: " << angle << std::endl;
    ...
    for(auto block : mOwner->GetGame()->GetBlocks()){
        ...
        if((collision == SideX1 ||collision == SideX2 || collision == SideY1 || collision == SideY2)){
            if(CanWallClimb(collision)){
                std::cout << "Can wall Climb: ";
                mWallClimbTimer = 0.0f;
                ChangeState(WallClimb);
                return;
            }else if(CanWallRun(collision)){
                std::cout << "Can Wall Run:";
                mWallRunTimer = 0.0f;
                ChangeState(WallRun);
                return;
            }
